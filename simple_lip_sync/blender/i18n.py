...
ZH_CN = {
    ...
    "... and {count} more": "... 以及 {count} 个",
    "No audio strips found on channel {channel}": "在通道 {channel} 上未找到音频片段",
    ...
    "Start Frame": "起始帧",
    "Timeline": "时间线",
    "{count} audio strip(s) found on channel {channel}": "在通道 {channel} 上找到 {count} 个音频片段",
    "Unknown audio source: {source}": "未知音频来源：{source}",
    "Use all audio strips on a sequencer channel": "使用序列器中某个通道上的所有音频片段",
    "Sequencer channel to collect audio strips from": "要获取音频片段的序列器通道",
    "Tuning Preset": "调参预设",
