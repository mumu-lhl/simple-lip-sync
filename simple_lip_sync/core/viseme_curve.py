"""Viseme scoring and sparse keyframe generation."""
...
def zero_weights():
    """Return an empty viseme weight map."""
...
def compute_openness(frame_db, frame_rms, db_threshold, rms_threshold):
    """Convert RMS/dB energy metrics to normalized mouth openness."""
...
def score_visemes(f1, f2):
    """Score visemes from estimated first and second formants."""
...
def score_visemes_from_formant_energy(formant_energy):
    """Score visemes from energy measured around each prototype formant pair."""
    if not formant_energy:
        return zero_weights()
...
def cap_total_weight(weights, limit):
    """Limit accumulated viseme values to the current openness envelope."""
...
def _simplify_track(track, start_frame):
    ...
    points = _deduplicate_points(_inject_boundaries(track, start_frame))
    if len(points) <= 2:
        return points
